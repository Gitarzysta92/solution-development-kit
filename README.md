...
- **Argo CD** is used in a “platform pulls from this repo” style. The **argocd/bootstrap** path is the integration-claim entrypoint and includes this repo AppProject + root app resources. In `threesixty-platform`, operators generate an integration claim `Application` (via `scripts/generate-platform-integration-claim.sh`) that points to this repo `argocd/bootstrap`. The **argocd/applications** directory contains the repo-owned app definitions (e.g. ARC runner config under `environments/dev/platform/...`). So the platform cluster syncs runner and related config from the solution-development-kit repo; the SDK does not drive the whole platform, only the parts that belong to the solution.
